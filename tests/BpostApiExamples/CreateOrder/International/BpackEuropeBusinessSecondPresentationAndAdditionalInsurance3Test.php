...
class BpackEuropeBusinessSecondPresentationAndAdditionalInsurance3Test extends PHPUnit_Framework_TestCase
{
    ...
    public function testCreateFromXml()
    {
        ...
        $this->assertCount(2, $internationalBox->getOptions());

        $options = $internationalBox->getOptions();

        $this->assertInstanceOf('Bpost\BpostApiClient\Bpost\Order\Box\Option\AutomaticSecondPresentation', $options[0]);

        /** @var Insured $insurance */
        $insurance = $options[1];
        $this->assertInstanceOf('Bpost\BpostApiClient\Bpost\Order\Box\Option\Insured', $insurance);
        $this->assertSame(Insured::INSURANCE_TYPE_ADDITIONAL_INSURANCE, $insurance->getType());
        $this->assertSame(Insured::INSURANCE_AMOUNT_UP_TO_5000_EUROS, $insurance->getValue());
    }
